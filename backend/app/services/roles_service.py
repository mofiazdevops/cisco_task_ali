...
class RolesService:
    ...
    def create_role_type(self, role_type_data: RoleCreate):
        try:
            is_role_name_valid = is_valid_name(role_type_data.name)

            if not is_role_name_valid:
                raise ValueError("Invalid role name.") 
            return self.roles_repository.create_role_types(role_type_data);
